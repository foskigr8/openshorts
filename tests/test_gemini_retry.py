...
def test_rotates_past_an_invalid_key(monkeypatch):
    # One dead key in the pool (401 UNAUTHENTICATED / ACCESS_TOKEN_TYPE_
    # UNSUPPORTED) used to kill the whole call — it must rotate to the next
    # key instead (confirmed in prod: 4 keys configured, one bad, context
    # died with 'invalid authentication credentials').
    monkeypatch.setenv("GEMINI_API_KEY", "good")
    monkeypatch.setenv("GEMINI_API_KEYS", "bad")
    calls = {"n": 0}
    ...
    monkeypatch.setattr(picker.gemini_pool, "generate_with_fallback",
                        fake_generate)
    parsed, _cost = picker._call_gemini("good", "prompt")
    assert calls["n"] == 2
    assert parsed["clips"][0]["start"] == 0
